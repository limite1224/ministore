     ...
     <form class="form-horizontal" action=""   method="post">	 
		...
		<#if entity?? && entity.customPropertyMap??>
			<#list entity.customPropertyMap?keys as key>
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="remark">${key!}： </label>
					<div class="col-sm-9">
		...
		<#elseif entity?? && entity.customPropertyList??>
			<#list entity.customPropertyList as property>
				<div class="form-group">
					<label class="col-sm-3 control-label no-padding-right" for="remark">${property.productMsgName!}(${property.productMsgKey!})： </label>
					<div class="col-sm-9">
						--
					</div>
				</div>
        ...
        <div class="form-group">
			...
			<button type="button" onclick="alert('权限不足');" class="btn btn-sm btn-danger"><i class="icon-lock align-top bigger-125"></i>
				一键上锁
			</button>&nbsp;&nbsp;&nbsp;
	...
	function getHistoryData(){
		var data = {
			...
			$.ajax({
				type:'post',
				data:data,
				url:'${web_base}/user/device_info!history.htm',
				dataType:'text',
